...
if gcloud compute instances describe "$VM" --zone "$ZONE" --project "$PROJECT" >/dev/null 2>&1; then
  ...
  gcloud compute instances start "$VM" --zone "$ZONE" --project "$PROJECT" 2>/dev/null || true
else
...
for i in $(seq 1 60); do
  if gcloud compute ssh "$VM" --zone "$ZONE" --project "$PROJECT" --tunnel-through-iap=false \
