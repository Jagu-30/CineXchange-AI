...
## Second backend implementation (`cinex/`, `services/`)

This repository also contains an independent backend built to the same product spec,
living at the repo root rather than under `backend/`. It is a seven-container Docker
Compose topology with five FastMCP agent services, Postgres as system of record and
ClickHouse as an analytics write-through.

- Setup and usage: [README-cinex-backend.md](README-cinex-backend.md)
- Design spec: [docs/superpowers/specs/](docs/superpowers/specs/)
- API contract: [docs/API.md](docs/API.md)

The two backends are not wired together and expect different environment variables.
The team still needs to choose one before the demo.
